...
def main():
    ...
    if args.T > 0:
        Xmix = Xmix[:, :, :args.T]; Ytrue = Ytrue[:, :, :args.T]; Ypred = Ypred[:, :, :args.T]
    if args.max_items > 0:
        Xmix = Xmix[:args.max_items]; Ytrue = Ytrue[:args.max_items]; Ypred = Ypred[:args.max_items]
    assert Xmix.ndim == 3 and Ytrue.ndim == 3 and Ypred.ndim == 3
    N, C, T = Ytrue.shape
    assert Xmix.shape == Ytrue.shape == Ypred.shape, (Xmix.shape, Ytrue.shape, Ypred.shape)
