...
class PRIDWT1D(layers.Layer):
    """
    Exact transpose of PRDWT1D analysis:
      analysis: tf.nn.conv1d(x, H, stride=2, padding="SAME")
      synthesis: tf.nn.conv1d_transpose(z, H, stride=2, padding="SAME")

    Important:
      - tf.nn.conv1d is cross-correlation, not mathematical convolution
      - so for the transpose operator, use conv1d_transpose with the SAME taps
      - do not reverse taps manually here
    """
    ...
    def call(self, inputs):
        ...
        # SAME taps as analysis
        H0 = _diag_filter_1d(h0, dwt.channels)   # [K, C, C]
        H1 = _diag_filter_1d(h1, dwt.channels)   # [K, C, C]
        ...
        out_len = T2 * 2
        out_shape = tf.stack([B, out_len, C])

        xa = tf.nn.conv1d_transpose(
            a,
            filters=H0,
    ...
    def compute_output_shape(self, input_shape):
        a_shape, d_shape = input_shape
        return (a_shape[0], None if a_shape[1] is None else a_shape[1] * 2, a_shape[2])
