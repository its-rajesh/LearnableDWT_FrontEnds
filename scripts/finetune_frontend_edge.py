#!/usr/bin/env python3
"""Small waveform/STFT frontend finetune on the disjoint edge distribution.

Matches finetune_fixed_wavelet_edge.py:
...
def make_dataset(x, y, batch_size, crop_len=None, aligned_to=1, shuffle=True, seed=1337):
...
def checkpoint_for(method: str, project: Path) -> Path:
    mapping = {
        "wave": project / "runs_ablation" / "20260225_181051_wave" / "best.keras",
        "stft": project / "runs_ablation" / "20260226_000354_stft" / "best.keras",
    }
    ckpt = mapping[method]
    if not ckpt.exists():
...
def aligned_to(method: str) -> int:
    return {"wave": 1, "stft": 256}[method]


def display_name(method: str) -> str:
    return {"wave": "Waveform", "stft": "STFT"}[method]
...
def main() -> None:
    ...
    ap.add_argument("--out_root", type=Path, default=PROJECT / "runs_frontend_finetune_edge_m9db_disjoint")
    ap.add_argument("--methods", nargs="+", default=["wave", "stft"], choices=["wave", "stft"])
    ...
    for method in args.methods:
        ckpt = checkpoint_for(method, args.project)
        out_dir = run_root / method
        out_dir.mkdir(parents=True, exist_ok=True)
        print(f"\n===== Finetune {method} from {ckpt} =====", flush=True)

        tr_ds = make_dataset(
            x_tr, y_tr, args.batch, crop_len=args.train_T,
            aligned_to=aligned_to(method), shuffle=True, seed=args.seed,
        )
        va_ds = make_dataset(
            x_va, y_va, args.batch, crop_len=None,
            aligned_to=aligned_to(method), shuffle=False, seed=args.seed,
        )
        ...
        for start in range(0, len(x_test), args.batch):
            ...
            pred = best.predict(x_test[start:end], verbose=0).astype(np.float32)[:, :, :args.eval_T]
            y_pred[start:end, :, :pred.shape[-1]] = pred
        ...
        rows.append(parse_metrics(metrics_dir, display_name(method)))

    csv_path = run_root / "frontend_finetune_edge_m9db_summary.csv"
